...
import { ArrowLeft, Sparkles, Palette, Layers, User, MessageCircle, Save, Loader2 } from 'lucide-react';
...
export default function AstrologyResultPage() {
    ...
    const [showNodeSignatures, setShowNodeSignatures] = useState(false);
    ...
    // Load temp reading from LocalStorage
    useEffect(() => {
        const loadTempReading = async () => {
            try {
                const supabase = createClient();
                const user = await getUserProfile(supabase as any);
                setUserProfile(user);

                const saved = localStorage.getItem('temp_astrology_reading');
                if (saved) {
                    const tempReading = JSON.parse(saved);
                    setReadings([tempReading]);
                    setSelectedReading(tempReading);
                    setSelectedProfileId(tempReading.unique_id || null);

                    // Sync settings
                    const savedSettings = tempReading.settings || tempReading.chart_data?.user_metadata?.settings;
                    if (savedSettings) {
                        if (savedSettings.orbStrictness) setOrbStrictness(savedSettings.orbStrictness as OrbStrictness);
                        if (savedSettings.viewPerspective) setViewPerspective(savedSettings.viewPerspective as any);
                        if (savedSettings.showExtraPoints !== undefined) setShowExtraPoints(savedSettings.showExtraPoints);
                    }
                }
            } catch (e) {
                console.error("Failed to load temporary reading", e);
            } finally {
                setLoading(false);
            }
        };
    ...
    // Filter reading based on selected profile (Simplified for single result)
    useEffect(() => {
        if (!readings.length) return;
        const match = readings[0]; // Always default to the only reading
        if (match) {
    ...
    useEffect(() => {
        ...
        if (selectedReading._isLocal && initializedReadings.current.has(key)) {
            return;
        }
    ...
    }, [selectedReading?.id]); // Removed _isLocal dependency to avoid loops
    ...
    const handleRecalculate = async (newStrictness: OrbStrictness) => {
        ...
        try {
            ...
            // Persist settings to local storage only for this page
            localStorage.setItem('temp_astrology_reading', JSON.stringify(updatedReading));
        ...
        try {
            ...
            const saveOne = async (data: any, sys: 'western' | 'vedic') => {
                ...
                const payload = {
                    ...
                    date_of_birth: selectedReading.date_of_birth,
                    time_of_birth: selectedReading.time_of_birth,
                    birth_city: selectedReading.chart_data?.meta?.location?.city || '',
                    birth_state: selectedReading.chart_data?.meta?.location?.state || '',
                    birth_country: selectedReading.chart_data?.meta?.location?.country || '',
                    ...
                    chart_data: {
                        ...data,
                        user_metadata: {
                            profile_name: (selectedReading.profile_name || 'My Reading') + (sys === 'vedic' ? ' (Vedic)' : ''),
                            settings: selectedReading.settings
                        }
                    }
                };
            ...
            // Save Both Concurrently if available
            const results = await Promise.all([
                selectedReading.chart_data ? saveOne(selectedReading.chart_data, 'western') : Promise.resolve(true),
                selectedReading.vedic_data ? saveOne(selectedReading.vedic_data, 'vedic') : Promise.resolve(true)
            ]);

            if (results.every(r => r)) {
                toast({ title: "Success", description: "Astrology charts saved to your profile!" });
                // Optional: Update reading state to no longer be local if we want to hide button
                // but usually user might want to save again or updates logic handles it.
            } else {
                ...
                <div className="relative z-10 max-w-[1700px] mx-auto px-6 pt-24 pb-12 md:pt-32 md:pb-20">
                    <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mb-8">
                        <a
                            ...
                            className="group inline-flex h-12 items-center justify-center gap-2 rounded-full bg-indigo-600 px-8 text-sm font-medium text-white transition-all hover:bg-indigo-500 hover:scale-105 hover:shadow-[0_0_20px_rgba(99,102,241,0.5)]"
                        >
                            <MessageCircle className="h-4 w-4 transition-transform group-hover:rotate-12" />
                            Join Discord Community
                        </a>

                        {(selectedReading?._isLocal || !selectedReading) && (
                            userProfile ? (
                                <button
                                    onClick={handleSave}
                                    disabled={isSaving || !selectedReading}
                                    className="group inline-flex h-12 items-center justify-center gap-2 rounded-full bg-white/10 border border-white/20 px-8 text-sm font-medium text-white transition-all hover:bg-white/20 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    {isSaving ? (
                                        <>
                                ...
                                <Link
                                    href="/signin"
                                    className="group inline-flex h-12 items-center justify-center gap-2 rounded-full bg-white/10 border border-white/20 px-8 text-sm font-medium text-white transition-all hover:bg-white/20 hover:scale-105"
                                >
                    ...
                    <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-10 gap-6 border-b border-white/5 pb-8">
                        ...
                        <div className="flex flex-wrap items-center gap-6 bg-white/[0.02] p-3 rounded-2xl border border-white/5 backdrop-blur-xl shadow-2xl">
                            ...
                            <Link href="/reading/astrology" className="p-3 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 text-gray-400 hover:text-white transition-all shadow-lg group">
                        ...
                        <div className="text-center py-24 bg-white/[0.02] border border-dashed border-white/10 rounded-3xl animate-fade-in-up">
                            <div className="max-w-md mx-auto">
                                ...
                                <p className="text-gray-500 mb-8 leading-relaxed">
                                    We couldn&apos;t retrieve your reading functionality. Please try generating it again.
                                </p>
                                <Link
