            ...
            <div>
                ...
                <div className="md:hidden space-y-3">
                    {filteredPlanets.map((planet: PointData, i: number) => {
                        ...
                        const planetAspects = chartData.aspects
                            .filter((a: any) => {
                        ...
                        return (
                            <div key={i} className="bg-[var(--bg-card)] border border-[var(--border-card)] rounded-xl p-4 shadow-lg">
                                {/* Top Row: Planet & Sign */}
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="w-8 flex justify-center shrink-0">
                                            {planet.type === 'star' ? (
                                                ...
                                                <span className={`text-2xl ${SIGN_COLORS[planet.sign] || 'text-[var(--text-muted)]'}`} style={{ fontFamily: '"Segoe UI Symbol", "Apple Color Emoji", serif' }}>
                                                    {SYMBOLS[planet.name] || ''}
                                                </span>
                                            )}
                                        </div>
                                        <div>
                                            <div className="font-bold text-base text-white">{planet.name}</div>
                                            <div className="text-xs text-[var(--text-secondary)] font-mono">
                                                {Math.floor(planet.deg)}°{String(Math.floor(planet.min)).padStart(2, '0')}&apos; {planet.sign}
                                            </div>
                                        </div>
                                    </div>

                                    {/* Sign Badge */}
                                    <div className={`px-2 py-1 rounded text-xs font-bold uppercase tracking-wider ${SIGN_COLORS[planet.sign] || 'text-white'} bg-white/5 border border-white/10`}>
                                ...
                                <div className="flex flex-wrap gap-2 mb-4">
                                    {!isAngle && (
                                        <div className={`text-[10px] font-black ${elementColor} uppercase tracking-tight flex items-center gap-1 bg-white/5 px-2 py-1 rounded border border-white/5`}>
                                            <span className={`w-1.5 h-1.5 rounded-full ${element === 'Fire' ? 'bg-orange-500' : element === 'Water' ? 'bg-blue-400' : element === 'Earth' ? 'bg-emerald-400' : 'bg-amber-300'}`} />
                                            {element}
                                        </div>
                                    )}
                                    {planet.isRetro && (
                                        <span className="text-[10px] text-rose-400 uppercase tracking-widest border border-rose-500/30 px-2 py-1 rounded font-black bg-rose-500/10">Rx</span>
                                    )}
                                    {!isAngle && dignityInfo && dignityInfo.score !== 0 && (
                                        <span className={`inline-flex items-center gap-1 px-2 py-1 rounded text-[10px] uppercase font-black tracking-widest border ${dignityInfo.score > 0
                                ...
                                {/* Bottom Row: Scrolly Aspects */}
                                {planetAspects.length > 0 && (
                                    <div className="border-t border-white/10 pt-3">
                                        <div className="text-[10px] uppercase tracking-widest text-[var(--text-secondary)] mb-2 font-bold">Aspects</div>
                                        <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
                                            {planetAspects.map((asp: any, idx: number) => {
                                                ...
                                                return (
                                                    <div key={idx} className={`flex-shrink-0 flex flex-col items-center justify-center px-3 py-2 bg-[var(--bg-main)]/50 rounded-lg border border-[var(--border-card)] border-white/5 ${isGhost ? 'opacity-40 border-dashed' : 'opacity-100'}`}>
                                                        <div className="flex items-center gap-1.5 text-xs font-bold text-white mb-0.5">
                                                            <span style={{ color: cfg.color }}>{cfg.symbol}</span>
                                                            <span>{SYMBOLS[other] || other.substring(0, 2)}</span>
                                                        </div>
                                                        <div className="text-[9px] text-[var(--text-secondary)] font-mono">{asp.orb.toFixed(1)}°</div>
                                                    </div>
                                                );
                                            })}
                                        </div>
                ...
                <div className="hidden md:block overflow-x-auto rounded-2xl border border-[var(--border-card)] bg-[var(--bg-card)] shadow-xl backdrop-blur-md custom-scrollbar">
